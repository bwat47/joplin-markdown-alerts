...
- Provides a `>!` autocomplete trigger: typing `>!` at the start of a line shows a dropdown of all alert types; selecting one inserts `> [!TYPE] ` with the cursor after the trailing space
...
- `src/contentScripts/codeMirror/alerts/alertAutocomplete.ts` - CM6 completion source for the `>!` alert-type dropdown plus its icon theme
...
- The `enableAlertAutocomplete` boolean setting (default `true`) controls the `>!` autocomplete; it is fetched once via `context.postMessage` when the CodeMirror content script initialises, so changes take effect when the note is reopened
